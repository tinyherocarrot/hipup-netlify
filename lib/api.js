...
export async function getCurrentProjects() {
  const entries = await fetchGraphQL(
    `query currentProjects: currentProjectsCollection(order: projectName_DESC) {
      items {
        projectName
        projectDescription {
          json
...
export async function getAboutContent() {
  ...
  return {
    missionStatement: entry?.data?.homepageCollection?.items[0].missionStatement.json,
...
export async function getCommunityEvents() {
  ...
  return entry?.data?.communityEventsCollection?.items;
}
